...
**가장 먼저 AudioListener를 확인하세요.** Unity는 씬에 `AudioListener`가 하나도 없으면 어떤 소리도 재생하지 않습니다. 기본 Main Camera에는 AudioListener가 붙어 있지만, UI만 있는 게임이나 카메라를 코드로 생성하는 경우 AudioListener가 빠져 "소리가 전혀 안 나는" 상태가 됩니다. 런타임에 `FindFirstObjectByType<AudioListener>() == null`이면 하나 추가하세요. 이건 무음 스위치·세션 설정과 무관하게 소리를 완전히 죽이므로, 세션 카테고리부터 손대기 전에 이것부터 봅니다.

AudioListener가 있는데도 안 들리면, **iOS 무음(링/사일런트) 스위치** 때문일 수 있습니다. 게임·미디어 앱은 보통 무음 스위치를 무시하도록 `AVAudioSession`을 **Playback 카테고리**로 설정해야 합니다. Unity 기본값은 Ambient(무음 스위치를 따름)라, 무음 모드에서 소리가 사라집니다. 작은 네이티브 코드(`.mm`)로 세션 카테고리를 Playback으로 바꾸면 해결됩니다. "사운드가 없다"는 피드백의 상당수는 음원 문제가 아니라 이 세션 설정 문제입니다.
...
## XCTest 병렬 실행에서 print가 사라짐

Xcode 병렬 테스팅(클론 시뮬레이터)에서는 테스트 안의 `print` 출력이 xcodebuild 로그에 나타나지
않는 경우가 많습니다. 진단 출력이 필요하면 **파일로 덤프**하세요
(`try? lines.joined().write(toFile: "/tmp/diag.txt", ...)`). 실패 메시지 본문이 필요할 때는
`-resultBundlePath`로 xcresult를 받아 `xcrun xcresulttool get test-results tests`를 파싱하면
assertion 메시지를 확정적으로 얻습니다.

## 이미지 생성 API의 배경 제거 옵션이 텍스처를 지우는 함정

스프라이트용 `no_background:true`(배경 투명화)를 **타일/텍스처 생성에 그대로 쓰면 "패널 자체"가
배경으로 판정돼 거의 전부 투명한 PNG**가 옵니다(실측: 4096px 중 22px만 불투명). 화면에서는
"타일이 안 보인다"로 나타나 렌더링 버그처럼 보입니다. 전체 캔버스를 채우는 텍스처는 배경 제거를
끄고, 생성 직후 **불투명 픽셀 수와 평균 휘도를 스크립트로 검사**해 파이프라인에서 걸러내세요.

## 거리 기반 트리거는 미로에서 벽을 관통한다

"플레이어가 N타일 안에 오면 보스 개전" 같은 근접 트리거는 개방 홀에서는 문제없지만, 통로가
겹겹인 미로에서는 **벽 너머 위층 복도에서 발동**합니다. 증상: AI가 도달 불가능한 목표점을 향해
벽에 영구 밀착(스톨), 원거리 공격이 벽에만 소모. 거리 검사에 **시야선(line-of-sight) 샘플링**을
추가하고, 같은 규칙을 판정·활성화·타게팅 전부에 공유하세요. 이 버그는 사람 플레이보다
**결정론 리플레이 봇의 완주 테스트**가 먼저 잡습니다 — 하한 검증 봇을 CI에 두는 이유입니다.

## 브랜치 리허설의 미커밋 변경이 checkout을 타고 넘어온다

"브랜치에서 플래그를 끄고 검증 → main 복귀 → 브랜치 삭제" 리허설 패턴의 함정: **끈 상태가
미커밋이면 `git checkout main`이 그 변경을 그대로 들고 넘어오고**, 직후의 `git add -A` 기록
커밋에 몰래 실린다(실측: 심사용 킬 스위치 OFF가 메인에 커밋돼 다음 배포에 탑재될 뻔).
리허설 브랜치에서는 변경을 **커밋해 두고**(브랜치 삭제로 함께 소멸) 넘어오거나, 복귀 직전
`git checkout -- <파일>`로 명시 복원하세요. 탐지 팁: 커밋 전 `git diff --cached`에서 의도치
않은 파일이 보이면 즉시 의심 — 특히 "한 줄 스위치"류는 diff가 짧아 눈에 잘 안 띈다.
