...
`simctl spawn <기기> defaults write <번들ID> ...`로 쓴 값은 **앱을 삭제해도 남는다**
(앱 컨테이너가 아니라 기기 레벨 cfprefsd 도메인에 저장됨). "신규 설치 첫 실행" 검증 시
uninstall만 하면 온보딩 플래그가 살아 있어 첫 실행 플로우가 스킵된다 — 반드시
`simctl spawn <기기> defaults delete <번들ID>`로 도메인을 지우고 검증하라.
...
스폰 지점에서 시각 검증을 하면 튜토리얼 배너·HUD 같은 **상위 z 레이어가 검증 대상(월드 레이어)을
정확히 덮을 수 있습니다**. 이펙트가 "안 나온다"고 코드부터 의심하기 전에, 검증 위치를 오버레이가
없는 개활 구간으로 옮겨(디버그 텔레포트 인자) 다시 캡처하세요. 실측 사례: 발사체가 완벽히
동작하는데 스폰 위치가 튜토리얼 배너 바로 아래라 모든 캡처에서 보이지 않았습니다.
런타임 로그(발사→소멸 카운트)로 로직 정상을 먼저 확정하면 원인 후보가 "렌더링 vs 가림"으로
좁혀집니다.

## 일시적(수명 <0.5초) 오브젝트는 정지 캡처 대신 영상 프레임 차분 (실측)

투사체·플래시처럼 수명이 짧은 오브젝트는 `simctl io screenshot` 연타로는 확률적으로만 잡힙니다.
`simctl io recordVideo`로 3~4초 녹화 후 AVFoundation 스크립트(ffmpeg 불필요 — macOS 기본)로
15fps 프레임을 뽑고, **연속 프레임 간 픽셀 차분**으로 "새로 나타난 색 픽셀"만 남기면 이동체가
확정적으로 검출됩니다. 정적 UI(같은 색 버튼 링, 펄스 애니메이션)가 오탐을 만들므로 알려진 UI
좌표는 마스킹하세요.

## simctl launch가 NotFound로 실패하면 번들 ID부터 확인 (실측)

`FBSOpenApplicationServiceErrorDomain code 4`에서 로그의 실제 사유가
`Application info provider returned nil`이면 **앱이 없는 게 아니라 번들 ID 오타**일 가능성이
큽니다. 부팅 타이밍·재설치를 반복하기 전에
`/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" <빌드산출물>/Info.plist` 로 실제 값을
확인하세요. 추측한 번들 ID로 수 분을 태우는 흔한 함정입니다.
