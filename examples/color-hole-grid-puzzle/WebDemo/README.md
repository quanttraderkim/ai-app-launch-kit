...
이 폴더는 `Docs/GameSpec.md`의 first playable proof를 브라우저에서 실행하는 정적 데모입니다. Play 화면은 `tutorial-001`만 플레이하며 정수 격자 상태, 한 칸씩 진행하는 직교 sweep, 슬롯별 수집, 부분 충전, 완료 제거, 승리와 재시작을 보여줍니다. Spread Audit은 후속 제작 예제인 `spread-demo-002`의 분산 수치를 읽기 전용으로 계산합니다. 자동 레벨 생성기와 독립 solver, Unity 모바일 빌드는 이 폴더의 구현 범위가 아닙니다.
...
브라우저 자동 검증은 `window.__PUZZLE_TEST__`의 snapshot, event log, state hash, reset, move, recorded replay와 distribution report를 읽을 수 있습니다. 이 hook은 테스트 관찰과 결정적인 명령 입력을 위한 것이며 DOM 위치나 애니메이션 callback으로 모델 상태를 수정하지 않습니다.
