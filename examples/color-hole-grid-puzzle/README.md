...
[`WebDemo`](WebDemo/README.md)는 이 문서 계약을 엔진과 무관한 JavaScript로 실행하는 작은 검증용 웹 앱입니다. Play 탭은 `tutorial-001`의 1×2 수집 트레이, 한 칸씩 검사하는 드래그, 부분 충전, 제거, 승리와 재시작을 구현하고, 기록 trace를 읽지 않는 독립 BFS가 같은 resolver로 최단 풀이를 찾습니다. Spread Audit 탭은 `spread-demo-002`를 플레이 레벨이나 자동 생성 결과라고 가장하지 않고, 색별 거리·구역과 전체 2×2·3×3 밀도 계산을 읽기 전용으로 보여줍니다.
...
퍼즐 algorithm을 논의할 때는 Docs/AlgorithmSystem.md를 기준으로 현재
color-hole 규칙과 선택적인 match-and-fall 규칙을 구분하라. 난이도는
solver의 raw metric vector로 보고하고 사용자 자료 없이 Easy/Hard를
임의 확정하지 마라. 현재 보드의 색 확률을 몰래 조작하지 말고 적응은
검증된 다음 레벨 선택이나 공개된 hint/assist로 설계하라.
